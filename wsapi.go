...
func (ch Channels) ToMap() map[string][][]int {
	channels := make(map[string][][]int)
	for _, c := range ch {
		ranges := [][]int{}
		for _, r := range c.Ranges {
			ranges = append(ranges, []int{r.Min, r.Max})
		}
		channels[c.ID] = ranges
	}
	return channels
}
...
type lazyRequestOp struct {
	Op   int             `json:"op"`
	Data lazyRequestData `json:"d"`
}

// LazyRequest requests guild members from the gateway
// The gateway responds back with GUILD_MEMBER_LIST_UPDATE type SYNC
// guildID   : Slice of guild IDs to request members of
// query     : String that username starts with, leave empty to return all members
// limit     : Max number of items to return, or 0 to request all members matched
// presences : Whether to request presences of guild members
func (s *Session) LazyRequest(guildID string, channels Channels, members *[]string, activities *bool, typing *bool, threading *bool) (err error) {
	...
	err = s.lazyRequest(data)
	return
}

func (s *Session) lazyRequest(data lazyRequestData) (err error) {
	s.log(LogInformational, "lazy request sent")
	...
	s.wsMutex.Lock()
	err = s.wsConn.WriteJSON(lazyRequestOp{14, data})
	s.wsMutex.Unlock()
